...
class ArduinoDevice(AqHardwareDevice, pyfirmata.Arduino):
        '''
        ...
        class Monitor(Thread):
            ...
            def _run(self, hardwareSystem: AqHardwareSystem, assignedBoard):
                ...
                :param 'assignedBoard': ArduinoDevice
                    Ссылка на объект исполнителя, с которым работает монитор.
                '''
                for unit in dict(assignedBoard.getPinMap(mode = object)).values():
                    try:
                        if type(unit) == ArduinoSensor:
        ...
        class FirmataStringResponse:
            ...
            def __init__(self, string: str):
                ...
                if len(string.split(';')) < 2:
                    raise InvalidFirmataStringResponseError()
        ...
        def __init__(self, comPort: str, isEnabled: bool,
                instanceName: str, desc: str, overrideDefaultTemplate: bool = False):
            '''
        ...
        def setChildren(self, _map: dict, drv: module) -> None:
            ...
            self.pinMap.update({'d:13': drv.Devices['A001'](self, 'd:13', isEnabled = True,
                                                              instanceName = 'D13Led',
...
class ArduinoSensor(AqHardwareDevice):
            ...
            while True:
                try:
                    if not self.statistic.isBusy:
                        ...
                        while self.statistic.isBusy: sleep(0.48)
                        self.statistic.registerStatistic(self.assignedSensor.getId(), self.assignedSensor.bakedValue())
                ...
                sleep(self.assignedSensor.probeFrequency)
    ...
    def __init__(self, atBoard: ArduinoDevice, atPin: str, isEnabled: bool,
        ...
        '''
        AqHardwareDevice.__init__(self, isEnabled, f'{atBoard.deviceAddress}:{atPin}:{driverId}', 
            'arduino', f'{atBoard.deviceAddress}:{atPin}',
            parent = atBoard, instanceName = instanceName, instanceDescription = desc)
        try: self.motherPin = self.parent.get_pin(f'{self.deviceAddress}:i')
...
class ArduinoExecutor(AqHardwareDevice):
    ...
    def __init__(self, atBoard: ArduinoDevice, atPin: str, isEnabled: bool,
        ...
        try: self.motherPin = self.parent.get_pin(f'{self.motherPinAddress}:i')
        except AttributeError: pass
