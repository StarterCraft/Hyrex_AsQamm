/*
    ...
    Copyright (C) 2021 Vladislaw Kuzmin. All rights reserved.
...
unsigned long charIndex(const char * findIn, const unsigned int symbol) {
...
void LEDo(const unsigned int effect, const unsigned int duration = 360) {
    //Команда осуществляет управление
...
void stringCallback(char * received) {
    //Обработать строку со строковой командой и вызвать метод,
...
void loop() {
    ...
    led.Update(); //тикаем LED, чтобы отвечал на команды
